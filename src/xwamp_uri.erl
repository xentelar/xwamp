...
is_valid(Uri) ->
  is_valid(Uri, xwamp_cfg:uri_strictness()).
...
%% @end
%% -----------------------------------------------------------------------------
-spec validate(Uri :: binary(), Arg :: rule() | match_strategy()) -> Uri :: t().
validate(Uri, Arg) ->
...
match(Uri, Uri, ?EXACT_MATCH) when is_binary(Uri) andalso byte_size(Uri) > 0 ->
  true;
...
match(Uri, Pattern, ?PREFIX_MATCH) when is_binary(Uri) ->
  binary:longest_common_prefix([Uri, Pattern]) >= byte_size(Pattern);

match(Uri, Pattern, ?WILDCARD_MATCH) when is_binary(Uri) andalso byte_size(Uri) > 0 ->
  subsumes(
    binary:split(Pattern, [<<$.>>], [global]),
    binary:split(Uri, [<<$.>>], [global])
  );

match(Uri, _, _) when is_binary(Uri) andalso byte_size(Uri) > 0 ->
  false;
...
%% @doc
%% Example:
%% components(<<"com.example.foo.bar">>) ->
%% [<<"com.example">>, <<"foo">>, <<"bar">>].
%% @end
%% -----------------------------------------------------------------------------
...
%% @end
%% -----------------------------------------------------------------------------
-spec uri_regex(atom(), atom() | re:mp()) -> re:mp().
...
uri_regex(prefix, undefined) ->
  Rule = case xwamp_cfg:uri_strictness() of
    loose -> loose_prefix;
...
%% @end
%% -----------------------------------------------------------------------------
-spec subsumes(Pattern :: binary() | [binary()], 
...
%% @end
%% -----------------------------------------------------------------------------
-spec maybe_error(atom(), term()) -> term().
...
%% @end
%% -----------------------------------------------------------------------------
-spec rule_for_strategy(term()) -> term().
rule_for_strategy(Strategy) ->
  rule_for_strategy(Strategy, xwamp_cfg:uri_strictness()).

%% -----------------------------------------------------------------------------
%% @doc
%% @end
%% -----------------------------------------------------------------------------
-spec rule_for_strategy(binary(), atom()) -> atom().
